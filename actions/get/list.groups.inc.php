...
include('./templates/head.inc.php');
  $items = user_listGroups();
  ?>
  <h1><?=$page['title'];?>
  <a href="./?a=add.group" class="btn btn-primary"><i class="fa fa-plus" aria-hidden="true"></i> добавить</a>
  </h1>
  <?
  ?>
  ...
  foreach($items as $k=>$v){
    ...
    <tr class="<? if(isset($_GET['id']) && $_GET['id'] == $v['_id']){echo "success";}?>">
      ...
      <td><?=$v['name'];?></td>
      <td><?=$v['_access'];?></td>
      <td><a href="./?a=edit.group&amp;id=<?=$v['_id'];?>" class="btn btn-default" title="редактировать"><i class="fa fa-pencil" aria-hidden="true"></i></a></td>
      <td><a href="./?a=remove.group&amp;id=<?=$v['_id'];?>" class="btn btn-danger" title="удалить"><i class="fa fa-times" aria-hidden="true"></i></a></td>
    </tr>
    <?php
  }
  ?></table><?
